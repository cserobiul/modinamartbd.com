...
<div class="body_scroll">
    ...
    <div class="container-fluid">
        @canany(['order.create'])
            <a class="btn btn-raised btn-primary btn-round waves-effect" href="{{ route('order.create') }}">New Purchase</a>
        @endcanany
        @canany(['order.all'])
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped table-hover dataTable js-exportable text-center">
                                        ...
                                        @foreach($orders as $key => $order)
                                            <tr>
                                                <td>{{ $key+1 }}</td>
                                                <td><a href="{{ route('order.show',$order->id) }}">{{ date('d M Y',strtotime($order->created_at)) }}</a>
                                                </td>
                                                <td>{{ $order->oid }}</td>
                            ...
                            @else
                                <div class="col-md-6">
                                    <h6 class="pt-3 text-danger"> No Purchase Found...
                                        @canany(['order.create'])
                                            <a class="btn btn-raised btn-primary btn-round waves-effect" href="{{ route('order.create') }}">Create New Purchase</a>
                                        @endcanany
                                    </h6>
                                </div>
                            @endif
