...
@section('title', isset($pageTitle) ?  $pageTitle : 'Purchase List')
    ...
    <div class="block-header">
        <div class="row">
            <div class="col-lg-7 col-md-6 col-sm-12">
                <h2> {{ isset($pageTitle) ?  $pageTitle : 'Purchase List' }}</h2>
                <ul class="breadcrumb">
        ...
        @canany(['purchase.all'])
            <div class="row clearfix">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="card">
                        <div class="header">
                            <h2><strong>Purchase List</strong></h2>
                        </div>
                        <div class="body" style="border: 2px solid #dddddd;">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped table-hover dataTable js-exportable text-center">
                                        ...
                                        @foreach($purchases as $key => $purchase)
                                            <tr>
                                                <td>{{ $key+1 }}</td>
                                                <td><a href="{{ route('purchase.show',$purchase->id) }}">{{ date('d M Y',strtotime($purchase->purchase_date)) }}</a>
                                                </td>
                                                <td>{{ $purchase->purchase_no  }}</td>
                                                <td>IN-{{ $purchase->invoice_no  }}</td>
                                                <td>{{ $purchase->purchase_amount }}</td>
                                                <td>{{ $purchase->pay_amount }}</td>
                                                <td>{{ $purchase->due_amount }}</td>
                                                <td>{{ $purchase->supplier->name }}</td>
                                                <td>{{ ucwords($purchase->user->name) }}</td>
                                                @canany(['purchase.update'])
                            ...
                            @else
                                <div class="col-md-6">
                                    <h6 class="pt-3 text-danger"> No Purchase Found...
                                        @canany(['purchase.create'])
