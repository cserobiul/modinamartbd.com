...
@section('title', isset($pageTitle) ?  $pageTitle : 'New Role')
    ...
    <div class="container-fluid">
        <div class="row clearfix">
            <div class="col-sm-12">
                <div class="card">
                    <div class="header">
                        <h2><strong>{{ isset($pageTitle) ?  $pageTitle : 'New Role' }}</strong></h2>
                        @canany(['role.all'])
                            <a class="btn btn-raised btn-primary btn-round waves-effect" href="{{ route('role.index') }}">Role List</a>
                        @endcanany
                    </div>
                    @include('backend.layouts._alert')
                    <div class="body" style="border: 2px solid #dddddd;">
                        <form id="form_advanced_validation" method="POST" action="{{ route('role.store') }}" enctype="multipart/form-data">
                            @csrf
                             <div class="card">
                                <div class="body">
                                    <div class="row clearfix">
                                        <div class="col-md-6 pb-2">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" name="name" value="{{ old('name') }}"  class="form-control" placeholder="Type Role Name" autofocus required>
                                            </div>
                                            @error('name')
                                            ...
                                            @foreach($permission_groups as $groups)
                                                ...
                                                <div class="col-md-3 role-{{ $i }}-management-checkbox">
                                                    ...
                                                    @foreach($permissions as $permission)
                                                        <div class="checkbox">
                                                            <input name="permissions[]" id="{{ $i.$j }}-Permission"  value="{{ $permission->name }}" type="checkbox">
                            ...
                            <button type="submit" class="btn btn-raised btn-primary btn-round waves-effect">Create Role</button>
                        </form>
                    </div>
                </div>
            </div>
